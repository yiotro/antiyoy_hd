...
public class AiBalancerDefaultV1 extends AbstractAI {
    ...
    public AiBalancerDefaultV1(CoreModel coreModel) {
        super(coreModel);
    ...
    private void killRedundantUnits(Province province) {
        int pricePeasant = getRuleset().getPrice(province, PieceType.peasant);
        int c = 0;
        while (c < 1000) {
            c++;
            if (!(province.getMoney() >= pricePeasant && coreModel.economicsManager.calculateProvinceProfit(province) >= 0)) break;
