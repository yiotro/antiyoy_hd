...
public class VisualTextContainer implements ReusableYio {
    ...
    private void moveViewList(RectangleYio parentPosition) {
        for (int i = 0; i < viewList.size(); i++) {
            if (i >= textList.size()) break; // two lists not synchronized for some reason
    ...
    private void syncViewListSize() {
        ...
        moveViewList(position); // to avoid null font problems
    }
    ...
    public void updateTextPosition() {
        ...
        if (textList.size() == 1 && textList.get(0).centered) {
